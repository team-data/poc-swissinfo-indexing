/**
 * See: https://beta.observablehq.com/@mbostock/d3-zoomable-sunburst
 */
function sunburstChart() {
    ...
    function chart(selection) {
        ...
        const label = g.append("g")
            ...
            .text(d => d.data.label.substr(0, 25));
        ...
        function showDetails(p) {
            d3.select(detailsContainer).html(
                `<h4>${p.parent.data.label}</h4>`+
                `<a href='${linkBaseUrl + p.data.label}'>${p.data.label}</a>`
            );
        }

        function clicked(p) {
            d3.select(detailsContainer).html(`<h4>${p.data.label}</h4>`);
